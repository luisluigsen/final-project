        ...
        <form action="<?=$url_action ?>" method="POST" enctype="multipart/form-data">
            <div class="mb-3 row">
                ...
                <div class="col-sm-1-12">
                    <input type="text" class="form-control " name="name" id="nameProduct" placeholder="Product Name" value="<?= isset($pro)&& is_object($pro) ? $pro->name :'';?>">
                </div>
            </div>
            <div class="mb-3 row">
                <label for="description" class="col-sm-1-12 col-form-label text-light">Description</label>
                <div class="col-sm-1-12">
                    <textarea class="form-control" name="description" id="descriptionProduct" placeholder="Description Name"><?= isset($pro)&& is_object($pro) ? $pro->description :'';?></textarea>
                </div>
            </div>
            <div class="mb-3 row">
                <label for="price" class="col-sm-1-12 col-form-label text-light">Price</label>
                <div class="col-sm-1-12">
                    <input type="number" class="form-control" name="price" id="priceProduct" placeholder="Price Product" value="<?= isset($pro)&& is_object($pro) ? $pro->price :'';?>">
                </div>
            </div>
            <div class="mb-3 row">
                <label for="stock" class="col-sm-1-12 col-form-label text-light">Stock</label>
                <div class="col-sm-1-12">
                    <input type="number" class="form-control" name="stock" id="stockProduct" placeholder="Stock Product" value="<?= isset($pro)&& is_object($pro) ? $pro->stock :'';?>">
                </div>
            </div>
            <div class="mb-3 row">
                ...
                <div class="col-sm-1-12">
                    <select class="form-control" name="category">
                        <?php while ($cat = $categories->fetch_object()) : ?>
                            <option value="<?= $cat->id ?><?= isset($pro)&& is_object($pro)&& $cat == $pro->category_id ? "Selected" :'';?>">
                                <?= $cat->name ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
